...
class TestHardware(utils.TestCommand):
    def setUp(self):
        ...
        self.app.client_manager.hardware = FakeHardwareClient(
            endpoint=fakes.AUTH_URL, token=fakes.AUTH_TOKEN
        )

        # Get a shortcut to the Hardware Mock
        self.app.client_manager.sdk_connection = mock.Mock()
        self.app.client_manager.sdk_connection.hardware = mock.Mock()
        self.sdk_client = self.app.client_manager.sdk_connection.hardware
...
class TestHardwareList(TestHardware):
    def setUp(self):
        ...
        self.cmd = hardware.ListResources(self.app, None)
...
class TestHardwareEnroll(TestHardware):
    def setUp(self):
        super(TestHardwareEnroll, self).setUp()
        self.cmd = hardware.EnrollResource(self.app, None)

    fake_hw = FakeHardware.create_one_hardware()

    def test_hardware_enroll(self):
        pass
